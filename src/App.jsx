...
function App() {
  ...
  }, [profiles, activeProfileId, transactions, accounts, bills, milesEntries]);
  ...
  const deleteProfile = (id) => {
    if (id === 'default') return alert('Não é possível excluir o perfil principal.');
    if (confirm('Tem certeza? Todos os dados vinculados a este familiar serão mantidos, mas ficarão sem perfil associado.')) {
      setProfiles(profiles.filter(p => p.id !== id));
      if (activeProfileId === id) setActiveProfileId('default');
    }
  };

  // Helper to get active profile info
  const activeProfile = profiles.find(p => p.id === activeProfileId) || profiles[0];

  // Logic Change: 
  // - Accounts, Transactions, Bills: Filter by profileFilter (PF/PJ/ALL)
  // - Miles: Filter by activeProfileId (family member)

  const filteredAccounts = accounts.filter(a => {
    const accountProfile = a.profile || 'PF'; // Default to PF if not set
    return profileFilter === 'ALL' || accountProfile === profileFilter;
  });

  const filteredTransactions = transactions.filter(t => {
    const transactionProfile = t.profile || 'PF'; // Default to PF if not set
    return profileFilter === 'ALL' || transactionProfile === profileFilter;
  });


  const filteredBills = bills.filter(b => {
    const billProfile = b.profile || 'PF'; // Default to PF if not set
  ...
  const handlePayBill = (bill, accountId) => {
    ...
    // Mark bill as paid instead of removing it
    ...
    alert(`Sucesso! Pagamento de ${bill.name} registrado na conta ${selectedAccount?.name}.`);
  };
    ...
    reader.onload = (e) => {
      try {
        ...
        if (data.transactions && data.accounts) {
          if (confirm('Isso substituirá todos os seus dados. Deseja continuar?')) {
    ...
    <div className={styles.appContainer}>
      ...
      <div style={{ marginTop: '3rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
          <h3 style={{ margin: 0 }}>Milhas da Família</h3>
        </div>

        {/* Profile/Family Switcher moved to Miles Section */}
          ...
          <div className={styles.profileList}>
            ...
            {profiles.map(p => (
              <div
                ...
                title="Botão direito para excluir"
              >
      ...
      <button className={styles.fab} onClick={() => { setEditingTransaction(null); setIsModalOpen(true); }} aria-label="Adicionar Transação">
        <Plus size={32} />
      </button>
